...
export const useVftRevokeMinterRoleMessage = (pairAddress: HexString) => {
  ...
  const { mutateAsync: revokeMinterRoleMessage, isPending } = useMutation({
    mutationFn: tx,
    onError: (error) => {
  ...
  return { revokeMinterRoleMessage, isPending };
};
