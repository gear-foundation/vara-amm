...
export const useVftGrantAdminRoleMessage = (pairAddress: HexString) => {
  ...
  const { mutateAsync: grantAdminRoleMessage, isPending } = useMutation({
    mutationFn: tx,
    onError: (error) => {
  ...
  return { grantAdminRoleMessage, isPending };
};
