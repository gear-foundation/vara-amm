/* eslint-disable */
// @ts-nocheck
import { GearApi, BaseGearProgram, type HexString, decodeAddress } from '@gear-js/api';
...
export class SailsProgram {
  ...
  newCtorFromCode(
    ...
    const builder = new TransactionBuilder<null>(
      ...
      undefined,
      'New',
      [pair_id, admin, fee_to, config],
      '([u8;32], [u8;32], [u8;32], Config)',
      'String',
      code,
      async (programId) => {
  ...
  newCtorFromCodeId(codeId: `0x${string}`, pair_id: CodeId, admin: ActorId, fee_to: ActorId, config: Config) {
    const builder = new TransactionBuilder<null>(
      this.api,
      this.registry,
      'create_program',
      ...
      if (getServiceNamePrefix(payload) === 'Factory' && getFnNamePrefix(payload) === 'PairCreated') {
        callback(
          this._program.registry
            ...
            .toJSON() as unknown as { token0: ActorId; token1: ActorId; pair_address: ActorId },
        );
      }
    });
  }
