...
export type NativeAccountState = { balance: bigint; nonce: bigint };
...
export class VoucherService implements OnModuleInit {
  ...
  async getNativeAccountState(account: string): Promise<NativeAccountState> {
    const api = await this.ensureConnected();
    const [balance, accountInfo] = await Promise.all([
      api.balance.findOut(account).then((value) => value.toBigInt()),
      (api as unknown as { query: { system: { account: (address: string) => Promise<unknown> } } }).query.system.account(account),
    ]);
    const nonce = this.extractNonce(accountInfo);
    return { balance, nonce };
  }

  private extractNonce(accountInfo: unknown): bigint {
    const direct = accountInfo as { nonce?: { toBigInt?: () => bigint; toString?: () => string } };
    if (direct.nonce?.toBigInt) return direct.nonce.toBigInt();
    if (direct.nonce?.toString) return BigInt(direct.nonce.toString());

    const json = (accountInfo as { toJSON?: () => unknown }).toJSON?.() as { nonce?: string | number } | undefined;
    if (json?.nonce !== undefined) return BigInt(json.nonce);

    throw new Error('Could not read account nonce');
  }
  ...
  /**
   * Additive update helper retained for future admin/internal flows. The AMM
   * onboarding service does not call this for automatic top-ups.
   */
  async update(
    voucher: Voucher,
    amountToAdd: number,
    ...
    // would let the next request read stale lastRenewedAt and repeat an
    // update. Retry 3× with backoff to cover transient DB
   ...
   * duration. This widens the whitelist when the frontend needs an active
   * voucher to cover another AMM program, without granting a second budget.
   *
   * No `lastRenewedAt` update (this is not a funding event), no budget charge,
   * no duration bump. Pure chain-side whitelist amendment.
   */
  ...
  async getAnyVoucher(account: string): Promise<Voucher | null> {
   ...
   * Used by the public GET /voucher/:account endpoint so the frontend can
   * show whether the one-time voucher still has budget.
   */
