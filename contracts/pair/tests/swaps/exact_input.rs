...
#[tokio::test]
async fn test_audit_atomicity_loss() {
    let actor_id = ActorId::from([1u8; 32]);
     let mut env = TestEnv::new(actor_id).await;
    
    let liquidity_amount = large_amount();
    let trader_funds = large_amount();

    env.setup_user(TRADER_1, trader_funds).await;
    env.setup_user(ACTOR_ID, U256::from(10) * liquidity_amount).await;
    ...
    let balance_a_before = env.token_client.balance_of(ACTOR_ID.into()).recv(env.token_a).await.unwrap();
    println!("💰 Balance A Before: {}", balance_a_before);

    // EXECUTE SWAP WITH FAULT INJECTION (666)
    println!("⚠️ Executing Swap(666) - Expecting Failure...");
    let res = env.pair_client
        .swap_exact_tokens_for_tokens(666.into(), 0.into(), true, env.get_deadline())
        .with_args(|args| args.with_actor_id(ACTOR_ID.into()))
        .send_recv(env.pair_id)
        .await;

    assert!(res.is_err(), "Swap should fail due to injected fault");

    // Check Balance After
    let balance_a_after = env.token_client.balance_of(ACTOR_ID.into()).recv(env.token_a).await.unwrap();
    println!("💰 Balance A After:  {}", balance_a_after);

    // VERDICT
    if balance_a_after < balance_a_before {
        let lost = balance_a_before - balance_a_after;
        println!("🚨 CRITICAL: Atomicity Broken! User lost {} tokens despite transaction failure.", lost);
        // This confirms the vulnerability
    } else {
        core::panic!("Test failed: Tokens were refunded? (Impossible with current code)");
    }
}